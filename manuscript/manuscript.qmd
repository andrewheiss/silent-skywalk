...
::: {#apptbl-sample-details}

```{r}
#| label: table-sample-details
#| include: true
...
base_table <- tbl_participant_summary %>% 
  group_by(category) %>% 
  gt()

base_table$`_data` <- base_table$`_data` %>% 
  mutate(Question = ifelse(!duplicated(Question), as.character(Question), ""))
...
#   ) %>%
#   pack_rows(
#     index = table(fct_inorder(participant_summary$category)),
...
amces_all_tbl <- amces_all %>% 
  ...
  mutate(nice = ifelse(is_reference, NA, glue("{fmt_decimal(y)}<br>{build_ci(ymin, ymax)}"))) %>% 
...
left_join(
  ...
  fmt_markdown(columns = c(mm_nice, amce_nice, contrast)) %>%
  cols_align(align = "center", columns = everything()) %>% 
...
## Additional marginal means and AMCEs
